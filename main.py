...
def crop_area(frame, polygon):
    ...
    mask = np.zeros(frame.shape[:2], dtype=np.uint8)           #--> (height, width, channels) 2--> (height, width)
...
def process_image(image, area_name):
    ...
    try:
        ...
        print(f"���️ Error processing image for {area_name}: {str(e)}")
...
# Function to porcess and print the results asynchronosly
